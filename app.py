...
def main():
    st.set_page_config(page_title="Bratislava 1 and 2-Room Listings", layout="wide")
    st.title("🏙️ Bratislava 1 & 2-Room Listings Map")
    ...
    for l in data:
        ...
        if not (l.get("latest_price") and min_price <= l["latest_price"] <= max_price):
            continue
        if show_changed and not l.get("price_changed"):
            continue
        if room_filter != "All" and room_filter not in category:
            continue
        filtered.append(l)
